...
def check_keydown_events(event, ai_settings, screen,  ship, bullets):
	...
	#	ship.moving_down = True		
		...
		sys.exit()
	#Смена космического корабля игрока
	elif event.key == pygame.K_1:
		ship.image = pygame.image.load('images/1.png').convert_alpha()
		ship.image = pygame.transform.scale(ship.image, ai_settings.size)
		ai_settings.bullet_color = (255, 0, 0)
	elif event.key == pygame.K_2:
		ship.image = pygame.image.load('images/2.png').convert_alpha()
		ship.image = pygame.transform.scale(ship.image, ai_settings.size)
		ai_settings.bullet_color = (0, 255, 100)
	elif event.key == pygame.K_3:
		ship.image = pygame.image.load('images/3.png').convert_alpha()
		ship.image = pygame.transform.scale(ship.image, ai_settings.size)
		ai_settings.bullet_color = (0, 0, 255)
	elif event.key == pygame.K_4:
		ship.image = pygame.image.load('images/4.png').convert_alpha()
		ship.image = pygame.transform.scale(ship.image, ai_settings.size)
		ai_settings.bullet_color = (48, 213, 200)	
	elif event.key == pygame.K_5:
		ship.image = pygame.image.load('images/5.png').convert_alpha()
		ship.image = pygame.transform.scale(ship.image, ai_settings.size)	
		ai_settings.bullet_color = (249, 166, 2)
...
def update_bullets(ai_settings, screen, stats, sb, ship, aliens, bullets):
	"""Обновляет позиции пуль  уничтожает старые пули."""
...
def create_fleet(ai_settings, screen, ship, aliens):
	...
	#Интервал между соседними пришельцами равен одной шиирне пришельца.
	alien = Alien(ai_settings, screen)
...
def update_aliens(ai_settings, screen, stats, sb, ship, aliens, bullets):
	"""
	Проверяет, не достиг ли флот края экрана,
...
def check_fleet_edges(ai_settings, aliens):
	"""Реагираует на достижение пришельцем края экрана."""
...
def ship_hit(ai_settings, screen, stats, sb, ship, aliens, bullets):
	"""Обрабатывает столкновение кораблся с пришельцем."""
...
def check_aliens_bottom(ai_settings, screen, stats, sb, ship, aliens, bullets):
	...
	for alien in aliens.sprites():
		if alien.rect.bottom >= screen_rect.bottom:
			#Происходит то же, что при столкновении с корбалем.
			ship_hit(ai_settings, screen, stats, sb, ship, aliens, bullets)
			break
